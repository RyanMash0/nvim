...
## Configuring
If you wish to add your own configurations, you should do so in the `<config_dir>/lua/user` directory. There are instructions for how to do this in the `<config_dir>/lua/user/init.lua` file. I use the dvorak keyboard layout, so the default leader key is "," (comma). For this reason, it is highly recommended to read the instructions for adding your own configurations, at least to set up your own leader key. If you do not set your leader key in the outlined manner, then none of the predefined keymaps will work. 
